...
public class DidMidnightResolverTests
{
    private static DidMidnightResolver CreateResolver(HttpMessageHandler handler)
    {
        var httpClient = new HttpClient(handler);
        var config = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["Midnight:ResolverUrl"] = "https://indexer.testnet.midnight.network"
            })
            .Build();
        return new DidMidnightResolver(httpClient, config, NullLogger<DidMidnightResolver>.Instance);
    }

    [Fact]
    public void CanResolve_MidnightDid_ReturnsTrue()
    {
        var resolver = CreateResolver(new FakeHandler(HttpStatusCode.OK, "{}"));
        Assert.True(resolver.CanResolve("did:midnight:testnet:abc123"));
    }
    ...
    public void CanResolve_OtherMethod_ReturnsFalse()
    {
        ...
        Assert.False(resolver.CanResolve("did:cheqd:mainnet:abc"));
    }

    [Fact]
    public void Method_ReturnsMidnight()
    {
        var resolver = CreateResolver(new FakeHandler(HttpStatusCode.OK, "{}"));
        Assert.Equal("midnight", resolver.Method);
    }
    ...
    public async Task Resolve_ValidResponse_ReturnsDidDocument()
    {
        var responseJson = """
        {
            "didDocument": {
                ...
                "id": "did:midnight:testnet:abc123",
                "verificationMethod": [
                    {
                        "id": "did:midnight:testnet:abc123#key-1",
                        "type": "Ed25519VerificationKey2020",
                        "controller": "did:midnight:testnet:abc123",
                        "publicKeyMultibase": "z6MkTest"
                    }
                ],
                "authentication": ["did:midnight:testnet:abc123#key-1"],
                "assertionMethod": ["did:midnight:testnet:abc123#key-1"]
            },
        ...
        var doc = await resolver.ResolveAsync("did:midnight:testnet:abc123");

        Assert.NotNull(doc);
        Assert.Equal("did:midnight:testnet:abc123", doc.Id);
    ...
    public async Task Resolve_404_ReturnsNull()
    {
        ...
        var doc = await resolver.ResolveAsync("did:midnight:testnet:nonexistent");
